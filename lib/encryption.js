...
class Secure {
  ...
  encryptData(data) {
    ...
    const validKey = this.#keyValidation(data.length, "😭🤣(/≧▽≦)/");
    const encryptData = [];
    ...
    return {
      split: this.#doubleCrypt(encryptData).join("."),
      raw: this.#doubleCrypt(encryptData).join(""),
    };
  }
  ...
  decryptData(data) {
    ...
    const validKey = this.#keyValidation(encryptData.length, "😭🤣(/≧▽≦)/");
    const decryptedData = [];
